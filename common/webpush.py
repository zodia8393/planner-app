...
def save_subscription(conn, profile_id: int, subscription_json: str):
    ...
    conn.execute(
        "INSERT OR REPLACE INTO push_subscriptions (profile_id, subscription_json, created_at) "
        "VALUES (?, ?, datetime('now', 'localtime'))",
        (profile_id, subscription_json),
    )
...
def remove_subscription(conn, profile_id: int, endpoint: str):
    ...
    conn.execute(
        "DELETE FROM push_subscriptions WHERE profile_id=? AND json_extract(subscription_json, '$.endpoint')=?",
        (profile_id, endpoint),
    )
