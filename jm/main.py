...
class SecurityHeadersMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request: Request, call_next):
        ...
        # CSP: 'unsafe-inline' required — 8 inline <script> blocks in base.html
        # + 62 inline event handlers (onclick/onchange) across templates.
...
async def method_not_allowed_handler(request: Request, exc):
    return HTMLResponse(_render_error_page(405, "허용되지 않는 요청입니다"), status_code=405)
...
async def http_exception_handler(request: Request, exc: HTTPException):
    ...
    return HTMLResponse(_render_error_page(exc.status_code, msg), status_code=exc.status_code)
    ...
    with get_db() as conn:
        ...
        for tbl in ("work_profiles", ):
            try:
                ...
                pass
        for tbl in ("todos", "events", "memos", "ddays", "work_logs", "notices", "categories"):
            try:
        ...
        # Check if the old UNIQUE constraint exists by trying an insert test
        try:
            # If the table has the old UNIQUE constraint, recreate it
            ...
            # Also check table SQL for inline UNIQUE
        ...
        # Seed default notification settings for profile 1
        existing_ns = conn.execute("SELECT COUNT(*) FROM notification_settings WHERE profile_id=1").fetchone()[0]
        if existing_ns == 0:
            import json as _json_init
            conn.execute("INSERT OR IGNORE INTO notification_settings (profile_id, target_type, offsets) VALUES (?,?,?)",
                         (1, "event", _json_init.dumps([{"value": 15, "unit": "minute"}])))
            conn.execute("INSERT OR IGNORE INTO notification_settings (profile_id, target_type, offsets) VALUES (?,?,?)",
                         (1, "todo", _json_init.dumps([{"value": 1, "unit": "day"}])))
            conn.execute("INSERT OR IGNORE INTO notification_settings (profile_id, target_type, offsets) VALUES (?,?,?)",
                         (1, "habit", _json_init.dumps([])))
            conn.execute("INSERT OR IGNORE INTO notification_settings (profile_id, target_type, offsets) VALUES (?,?,?)",
                         (1, "dday", _json_init.dumps([{"value": 1, "unit": "day"}])))
...
async def toggle_habit(request: Request, habit_id: int):
    ...
    log_date = form.get("date", "") or _date_cls.today().isoformat()
    ...
    with get_db() as conn:
        ...
        if tracking_type in ("times_per_day", "every_n_hours") and action in ("increment", "toggle"):
            ...
            # Use current time as log_time to avoid UNIQUE(habit_id, log_date) constraint
            from datetime import datetime as _dt_cls
            counter_time = _dt_cls.now().strftime("%H:%M:%S")
            if action == "toggle":
                # Check if already at target, if so remove last
...
async def increment_habit(request: Request, habit_id: int):
    ...
    # Use current time as log_time to avoid UNIQUE constraint
...
async def timetable_page(request: Request, dt: str = "", day_type: str = ""):
    ...
    with get_db() as conn:
        ...
        # Habits with time-based tracking
        habits = conn.execute(
            "SELECT * FROM habits WHERE profile_id=? AND archived=0 ORDER BY sort_order", (pid,)
        ).fetchall()
